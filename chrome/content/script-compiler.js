...
contentLoad: function(e) {
    ...
    if (eproc_gmCompiler.isGreasemonkeyable(href)) {
        var prefs = new eproc_PrefManager();
        var script = false;
        if (/https:\/\/jef[23]?\.jf(pr|rs|sc)\.(gov|jus)\.br\/eproc(V2|v2_homologacao|v2_apresentacao)\/controlador\.php\?acao=acessar_documento/.test(href)) {
        } else if (prefs.getValue('v2.enable') && /https:\/\/jef[23]?\.jf(pr|rs|sc)\.(gov|jus)\.br\/eproc(V2|v2_homologacao|v2_apresentacao)\/.*/.test(href)) {
...
var httpRequestObserver =
{
  observe: function(subject, topic, data)
  {
    if (topic == "http-on-examine-response") {
      ...
      if (/https:\/\/jef[23]?\.jf(pr|rs|sc)\.(gov|jus)\.br\/eproc(V2|v2_homologacao|v2_apresentacao)\/controlador\.php\?acao=acessar_documento/.test(httpChannel.name)) {
